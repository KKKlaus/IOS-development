...
class ProfileViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @IBAction func fullNameEditing(_ sender: Any) {
        guard  let user = Auth.auth().currentUser else { return }
    ...
    @IBAction func locationEditing(_ sender: Any) {
        guard  let user = Auth.auth().currentUser else { return }
        
        let id = user.uid
        let ref = Database.database().reference().child("Users").child(id).child("location")
        ref.setValue(txtLocation.text)
    }
    ...
    @IBAction func confirmChangeBtn(_ sender: UIButton) {
        upLaodProfilePic()
    }
